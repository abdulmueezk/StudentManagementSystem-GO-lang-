...
func (o *LoginUserURL) Build() (*url.URL, error) {
	...
	var _path = "/Teacher/login/{email}/{password}"

	email := o.Email
	if email != "" {
		_path = strings.Replace(_path, "{email}", email, -1)
	} else {
		return nil, errors.New("email is required on LoginUserURL")
	}
	...
	if password != "" {
		...
		return nil, errors.New("password is required on LoginUserURL")
	}
